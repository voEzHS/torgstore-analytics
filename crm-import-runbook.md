...
товаров/категорий), выгружается как Excel за календарный месяц целиком —
в отличие от накладных/рекламы, у этого отчёта нет режима «с ... по ...» на
произвольный диапазон дней, только помесячно. Для неполного месяца (например,
1-7 августа) выгружать можно, но нужно явно указывать в комментарии/этикетке,
что это ЧАСТИЧНЫЙ месяц, а не полный — иначе через 3 недели цифры будут
выглядеть как «обвал продаж» при сравнении с предыдущими полными месяцами.
